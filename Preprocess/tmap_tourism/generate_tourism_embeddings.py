"""
Generate LLM Embeddings from Tourism Prompts
Uses Llama 3.2-3B-Instruct to create embeddings from combined tourism data:
- Korean credit card sales (domestic tourism)
- Foreign credit card sales (international tourism)
- Navigation searches by category (destination interest)
"""
...
PROMPT_FILE = TOURISM_DIR / "tourism_prompts.csv"
OUTPUT_FILE = TOURISM_DIR / "tourism_llm_embeddings.csv"
BATCH_SIZE = 16
CHUNK_SIZE = 512
...
def main():
    print(f"{'='*100}")
    print(f"TOURISM LLM EMBEDDING GENERATION")
    print(f"{'='*100}\n")
    ...
    # Show sample
    print(f"\nSample prompt:")
    print(f"  Dong: {df['Dong_name'].iloc[0]}")
    print(f"  Month: {df['month'].iloc[0]}")
    print(f"  Length: {len(df['prompt'].iloc[0])} chars")
    print(f"  Text: {df['prompt'].iloc[0][:200]}...")
    ...
    # Convert month to proper format (YYYYMM -> YYYY-MM-DD)
    reporting_months = pd.to_datetime(df['month'], format='%Y%m').dt.strftime('%Y-%m-01')

    output_data = {
        'Reporting Month': reporting_months,
    ...
    print(f"\n{'='*100}")
    print(f"TOURISM EMBEDDING GENERATION COMPLETE")
    print(f"{'='*100}")
    ...
    print(f"\n{'='*100}")
    print(f"NEXT STEP: Filter to model period (2017-01 to 2021-11) and test with transformer model")
    print(f"{'='*100}")
